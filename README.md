...
│   ├── debate.py           # 辩论模式逻辑（正反方对抗）
│   ├── debate_state.py     # 辩论状态管理
│   ├── debate_prompts.py   # 辩论Prompt模板
│   ├── state.py            # 讨论状态管理
│   └── utils.py            # 公共工具函数
├── session/                # 会话模块
│   └── manager.py          # 会话持久化管理
...
**触发条件说明：**
- 圆桌讨论：必须包含"圆桌讨论"或"圆桌会议"
- 多AI并行：包含"大家"、"一起"、"你们"等关键词
...
```
用户: 辩论一下：微服务架构比单体架构更好

→ 🔸 正方(Claude): 微服务带来的优势...
→ 💠 反方(Gemini): 单体架构在特定场景更合适...
→ 双方激烈交锋，质询与反驳
→ ❇️ 评委(Codex): 综合评判，给出裁决
→ 输出完整辩论记录和评分
```
